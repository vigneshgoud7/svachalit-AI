...
function safeJsonParse<T>(value: string | undefined, fallback: T): T {
  ...
  try {
    ...
    console.warn('[AIOrchestrator] Failed to parse tool arguments:', error);
    return fallback;
  }
}

function cacheKeyFor(tenantId: string, body: string, contextText: string) {
  return crypto
    .createHash('sha256')
    ...
    if (!toolCallsTriggered && !cached) {
      await prisma.$executeRaw`
        ...
        VALUES (
          ...
          ${body},
          ${aiResponseText},
          ${new Date(Date.now() + env.RESPONSE_CACHE_TTL_SECONDS * 1000)}
        )
