...
const genAI = new generative_ai_1.GoogleGenerativeAI(env_1.env.GEMINI_API_KEY || 'dummy-key');
/**
 ...
 */
async function queryKnowledgeBase(tenantId, queryText, limit = 3) {
    try {
        const embedding = await generateEmbedding(queryText);
        const vectorStr = `[${embedding.join(',')}]`;
        // Query pgvector using PostgreSQL cosine distance operator `<=>`
        const results = await client_1.prisma.$queryRawUnsafe(`SELECT id, title, content, (1 - (embedding <=> $1::vector)) as similarity
       FROM "KnowledgeBase"
       WHERE "tenantId" = $2
       ORDER BY embedding <=> $1::vector
       LIMIT $3`, vectorStr, tenantId, limit);
        return results;
    }
    catch (error) {
        ...
        const fallbackResults = await client_1.prisma.knowledgeBase.findMany({
            where: {
                tenantId,
                OR: [
